...
<%//获得参数
    ...
    int step_id_s=Integer.parseInt(step_id)-1;
...
<tr>    
      ...
      <%
      int i=midObjectStepView.getMaxserino(i_biz_id,step_id_s);
      while(rs.next()){
      %>  
        <table id="table<%=i%>" width="116%" border="1" cellspacing="0" cellpadding="0" bordercolorlight='#6699cc' bordercolordark='#ECF5FF' class="t">   
          <tr>    
            <td height="26" width="93%">   
              <input type="text" name="name_ch" size=10 value="<%=rs.getString("input_lable").trim()%>">   
               
              <input type="text" name="explain" size=10 value="<%=rs.getString("input_memo").trim()%>">      
                
                 
              <input type="text" name="name_en" size=10 value="<%=rs.getString("input_name").trim()%>">      
                 
                  
              <select size="1" name="type_s">      
          	<option value="<%=rs.getString("input_type").trim()%>"><%=rs.getString("input_type").trim()%></option>
              ...
              event.returnValue=false; "  value="<%=rs.getInt("input_size")%>">
              <select size="1" name="word_type">      
          	<option value="<%=rs.getInt("word_type")%>">
          	<%
          	if(rs.getInt("word_type")==1){
          	%>
          	    混合型
          	<%
          	}
          	if(rs.getInt("word_type")==2){
          	%>
          	    数字型
          	<%
          	}
              ...
              <select size="1" name="check_sign">
          	<option value="<%=rs.getInt("check_sign")%>">
          	<%
          	if(rs.getInt("check_sign")==1){
          	%>
          	检测
          	<%
          	}
            ...
            <td width="7%" onclick="javascript:document.all.table<%=i%>.del()">删除 </td>  
          </tr>   
        </table>
      <%
      i--;
