...
<%
	...
	log.Write("###########签约 最终操作结果 回显界面########yiDongCharge4.jsp######begins");
	...
	String signNo = MessManTool.changeChar(request.getParameter("signNo"));//签约号码
...
<%
	...
	String content = "biz_id,32|biz_step_id,1|TXNSRC,WE441|SigTyp,"+signType+
								 "|VchNO,"+"|ActTyp,4"+"|ActNo,"+cdno+"|ActNam,"+ActNam+"|IdTyp,"+IdTyp+
								 "|IdNo,"+IdNo+"|CusNam,"+ActNam+"|MstTel,"+mainNo+
								 "|SigTel,"+signNo+"|SigFlg,"+signFlg+"|PINDat,"+bocomPwd;
	
	log.Write(">>>>>>>>>>>>移动签约>>>发出去的报文:"+content);
	MidServer midServer = new MidServer();
	BwResult bwResult = midServer.sendMessage(content);
	log.Write("<<<<<<<<<<<<移动签约<<<返回的报文对象bwResult:"+bwResult);
	
	if (bwResult == null ) {
			log.Write("交易结果不对");
			AppParam.setParam(dse_sessionId+"midErr1","签约操作失败");
